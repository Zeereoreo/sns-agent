...
def run_diagnostics(d) -> list[dict]:
    ...
    if not idx:
        ...
        # 🔴 제목 검색이 0 이라고 '블로그째 제외'로 단정하면 안 된다(2026-08-06 정정).
        # 같은 날 순위 수집에서 '비제이 피켓 제작' 2위가 잡혔다 — 옛 글 일부는 살아 있다.
        # 제목 검색이 0 인 이유는 7/29 에 발행글 20편의 제목을 통째로 교체했기 때문일 수
        # 있다(색인에 옛 제목이 남아 있으면 새 제목 정확검색은 당연히 0 이 된다).
        # 그래서 **순위 데이터라는 두 번째 증거**를 같이 본다: 순위가 하나라도 잡히면
        # 색인은 부분적으로 살아 있는 것이고, 그때 처방은 '전체 제외' 때와 달라진다.
        ranked = []
        try:
            _r = _load_json(ROOT / "data" / "metrics.json", {}).get("ranks") or {}
            if _r:
                _last = sorted(_r)[-1]
                ranked = [f"{k} {v}위" for k, v in (_r[_last] or {}).items() if v]
        except Exception:
            ranked = []
        if ranked:
            add("네이버 색인", "warn",
                f"제목 검색은 {idx.get('sampled')}편 모두 실패했지만 **순위는 잡힘**"
                f"({', '.join(ranked[:3])}) — 블로그째 제외가 아니라 **새 글이 색인 안 되는 상태**",
                "옛 글은 살아 있다. 제목 검색 0 은 7/29 제목 일괄교체 탓일 수 있으니 단정하지 말 것")
        else:
            add("네이버 색인", "bad",
