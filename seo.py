...
# 이미지 만점 기준 (2026-08-03 하향 — 20장은 **우리가 채울 수 없는 기준**이 됐다)
#   풀 519장 중 493장(95%)이 원본 블로그 사진이고, 그 복제가 색인 이탈의 원인 후보로
#   드러나 글당 3장으로 제한했다(images.ORIGIN_MAX_PER_POST). 실제 삽입은 4~5장이다.
#   20장을 기준으로 두면 모든 글이 같은 이유로 감점돼 게이트가 변별력을 잃는다.
#   8 = 갤러리 제거 후 본문 슬롯 실측(7~10, 중앙 8)이자 색인이 살아 있던 구간의 글 규격.
#   사용자 지시(2026-08-03): "사진 갯수에 집착 안 해도 되니까".
#   새 사진이 인박스에 들어오면 그때 원본 규격(20)으로 되돌린다.
IMG_GOOD = 8
